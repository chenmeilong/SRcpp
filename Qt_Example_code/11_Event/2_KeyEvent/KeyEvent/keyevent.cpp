...
{
    if(event->modifiers()==Qt::ControlModifier)			//(a)判断ctrl按键是否按下
    {
    ...
    else								//对【Ctrl】键没有按下的处理
    {
        ...
        if(event->key()==Qt::Key_Home)					//(j)home按键按下
        {
            startX=0;
            startY=0;
        }
        if(event->key()==Qt::Key_End)					//(k)end按键按下
        {
...
void KeyEvent::paintEvent(QPaintEvent *)        //界面重画函数
{
    QPainter painter;
    painter.begin(this);
