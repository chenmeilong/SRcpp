...
QVariant ModelEx::data(const QModelIndex &index, int role) const//函数返回指定索引的数据
{
    if(!index.isValid())
        return QVariant();
...
QVariant ModelEx::headerData(int section, Qt::Orientation orientation, int role) const//返回固定的表头数据，设置水平表头的标题
{
