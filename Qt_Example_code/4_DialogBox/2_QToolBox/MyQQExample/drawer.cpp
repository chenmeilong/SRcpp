...
Drawer::Drawer(QWidget *parent, Qt::WindowFlags f):QToolBox(parent,f)
{
    ...
    toolBtn1_1->setText(tr("张三"));						//(b)  设置按钮的名字
    toolBtn1_1->setIcon(QPixmap("11.png"));				//(c)   设置按钮的图标
    toolBtn1_1->setIconSize(QPixmap("11.png").size());	//(d)  设置按钮大小，本例将其设置为与图标的大小相同
    toolBtn1_1->setAutoRaise(true);						//(e)   当鼠标移开时，按钮恢复为弹起状态
    toolBtn1_1->setToolButtonStyle(Qt::ToolButtonTextBesideIcon);//设置按钮的 文字显示的位置  这里设置为文字显示在按钮的旁边
    ...
    QGroupBox *groupBox1=new QGroupBox;				//(g)创建一个   QGroupBox 实例，对应每一个抽屉
    QVBoxLayout *layout1=new QVBoxLayout(groupBox1);//(h)  创建一个QVBoxLayout实例 用来设置抽屉各个按钮的布局
    ...
    //将准备好的抽屉插入ToolBox中
    this->addItem((QWidget*)groupBox1,tr("我的好友"));   //将准备好的抽屉插入ToolBox中
